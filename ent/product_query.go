...
type ProductQuery struct {
	...
	withAttributeValuesString *AttributeValueStringQuery
	withAttributeValuesNum    *AttributeValueNumQuery
...
// QueryAttributeValuesString chains the current query on the "attributeValuesString" edge.
func (pq *ProductQuery) QueryAttributeValuesString() *AttributeValueStringQuery {
	query := (&AttributeValueStringClient{config: pq.config}).Query()
...
// QueryAttributeValuesNum chains the current query on the "attributeValuesNum" edge.
func (pq *ProductQuery) QueryAttributeValuesNum() *AttributeValueNumQuery {
	query := (&AttributeValueNumClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryAttributeValuesBool() *AttributeValueBoolQuery {
	query := (&AttributeValueBoolClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withAttributeValuesString: pq.withAttributeValuesString.Clone(),
		withAttributeValuesNum:    pq.withAttributeValuesNum.Clone(),
		withAttributeValuesBool:   pq.withAttributeValuesBool.Clone(),
...
// WithAttributeValuesString tells the query-builder to eager-load the nodes that are connected to
// the "attributeValuesString" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithAttributeValuesString(opts ...func(*AttributeValueStringQuery)) *ProductQuery {
	query := (&AttributeValueStringClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAttributeValuesString = query
	return pq
}

// WithAttributeValuesNum tells the query-builder to eager-load the nodes that are connected to
// the "attributeValuesNum" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithAttributeValuesNum(opts ...func(*AttributeValueNumQuery)) *ProductQuery {
	query := (&AttributeValueNumClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAttributeValuesNum = query
	return pq
}
...
func (pq *ProductQuery) WithAttributeValuesBool(opts ...func(*AttributeValueBoolQuery)) *ProductQuery {
	query := (&AttributeValueBoolClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withAttributeValuesBool = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [3]bool{
			pq.withAttributeValuesString != nil,
			pq.withAttributeValuesNum != nil,
			pq.withAttributeValuesBool != nil,
		}
	)
	if withFKs {
	...
	if query := pq.withAttributeValuesString; query != nil {
		if err := pq.loadAttributeValuesString(ctx, query, nodes,
			func(n *Product) { n.Edges.AttributeValuesString = []*AttributeValueString{} },
			func(n *Product, e *AttributeValueString) {
				n.Edges.AttributeValuesString = append(n.Edges.AttributeValuesString, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := pq.withAttributeValuesNum; query != nil {
		if err := pq.loadAttributeValuesNum(ctx, query, nodes,
			func(n *Product) { n.Edges.AttributeValuesNum = []*AttributeValueNum{} },
			func(n *Product, e *AttributeValueNum) {
				n.Edges.AttributeValuesNum = append(n.Edges.AttributeValuesNum, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := pq.withAttributeValuesBool; query != nil {
		if err := pq.loadAttributeValuesBool(ctx, query, nodes,
			func(n *Product) { n.Edges.AttributeValuesBool = []*AttributeValueBool{} },
			func(n *Product, e *AttributeValueBool) {
...
func (pq *ProductQuery) loadAttributeValuesString(ctx context.Context, query *AttributeValueStringQuery, nodes []*Product, init func(*Product), assign func(*Product, *AttributeValueString)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributevaluestring.FieldProductID)
	}
	query.Where(predicate.AttributeValueString(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.AttributeValuesStringColumn), fks...))
	}))
...
func (pq *ProductQuery) loadAttributeValuesNum(ctx context.Context, query *AttributeValueNumQuery, nodes []*Product, init func(*Product), assign func(*Product, *AttributeValueNum)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributevaluenum.FieldProductID)
	}
	query.Where(predicate.AttributeValueNum(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.AttributeValuesNumColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *ProductQuery) loadAttributeValuesBool(ctx context.Context, query *AttributeValueBoolQuery, nodes []*Product, init func(*Product), assign func(*Product, *AttributeValueBool)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(attributevaluebool.FieldProductID)
	}
	query.Where(predicate.AttributeValueBool(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.AttributeValuesBoolColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
