...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldLogin, user.FieldPassword, user.FieldFirstName, user.FieldLastName, user.FieldRole, user.FieldPhonenumber, user.FieldEmail, user.FieldToken, user.FieldRefreshToken:
			values[i] = new(sql.NullString)
		case user.FieldCreatedAt, user.FieldUpdatedAt, user.FieldDeletedAt:
			values[i] = new(sql.NullTime)
		case user.ForeignKeys[0]: // user_settings
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				u.user_settings = new(int)
				*u.user_settings = int(value.Int64)
			}
		default:
